...
typedef struct {
    ...
    //int8_t f1; /* face number in element 1 */
    //int8_t f2; /* face number in element 2 */
}
t8wyo_face_t;
...
static int
t8wyo_face_set_face2cell(void **face, const void *data){
    t8wyo_face_t *Face;
    external_t *ext;

    ext = (external_t *) data;
    Face = *(t8wyo_face_t **) face;
...
void t8wyo_build_lists(t8_forest_t forest,external_t *ext){
    ...
    //ext->elem_info.malloc(INFO_ELEM_SIZE*elem_count,-1); // auto-deletes if pre-allocated
    ...
    for (t8_locidx_t itree = 0,elem_index = 0; itree < num_local_trees; ++itree) {
        ...
        for (t8_locidx_t ielement = 0; ielement < num_elements_in_tree; ++ielement, ++elem_index) {
            ...
            //ext->elem_info[INFO_ELEM_SIZE*elem_index+ETYPE_IND] = get_class(tree_class);
            //ext->elem_info[INFO_ELEM_SIZE*elem_index+ELEVL_IND] = eclass_scheme->t8_element_level(element);
    ...
    while(faces->elem_count > 0){
        ...
        if(Face_full->num_neighbors > 0){
            ...
            for (int ielem = 0; ielem < Face_full->num_neighbors; ielem++) {
                ...
                //Face->f1 = Face_full->face_number;

                /* set neighbor element info */
                Face->e2 = Face_full->neighids[ielem];
                //Face->f2 = Face_full->dual_faces[ielem];

                /* try to insert the face into the hash */
                if(sc_hash_insert_unique(faces_unique,Face,NULL)) face_unique_count++;
    ...
    //wyo::memory<int> face_info(2*face_unique_count,-1);

    //sc_hash_foreach(faces_unique,t8_face_free);
